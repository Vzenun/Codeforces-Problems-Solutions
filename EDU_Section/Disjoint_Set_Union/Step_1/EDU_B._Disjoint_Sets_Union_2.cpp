    ...
    void union_set(ll a,ll b){
        ...
        if(a==b){
            ...
            if(size[a]>=size[b]){
                parent[b]=a;
                size[a] += size[b];
                maxels[a]=max(maxels[a],maxels[b]);
                minels[a]=min(minels[a],minels[b]);
            }
            else{
                parent[a]=b;
                size[b] += size[a];
                maxels[b]=max(maxels[a],maxels[b]);
                minels[b]=min(minels[a],minels[b]);
            }
        }
    }
};
...
void solve_array(){
    ...
    rep(i,0,m){
        ...
        if(s=="union"){
            ...
            cin>>a1;
            ll ar=ds->find_set(a1);
            cout<<ds->minels[ar]<<" "<<ds->maxels[ar]<<" "<<ds->size[ar]<<nn;
        }
    }
}

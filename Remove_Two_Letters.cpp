...
#define mst multiset<ll> // it contains multiple instances of the same value in ascending order
#define st set<ll> // it contains single instance of the same values in ascending order
...
void solve_single(){
    ...
    if(n==3){
        ...
        ll am=0;
        rep(i,1,n-1){
            if(i<n-2 && s[i]==s[i+2] && s[i-1]==s[i+1]){
                if(s[i]==s[i-1]){
                    am+=2;
                    i++;
                }

...
void solve_array(){
    ...
    ll sum=0;
    rep(i,0,n){
        ll ans=upper_bound(all(arr),r-arr[i])-arr.begin();
        // this will rreturn the index below which every element is <=r-arr[i]
        // and if all are in this than return n
        ans-=lower_bound(all(arr),l-arr[i])-arr.begin();
        // this will return the index above and at which every element is > or =l-arr[i]
        // and if all are in this than return n
        if(arr[i]*2>=l && 2*arr[i]<=r){
            ans--;
        }
        sum+=ans;
        //cout<<ans<<nn;

...
vector<vector<ll> > dp;
vpll arr;
ll dp_solve(ll n,ll previous,ll num,ll signal){
    if(n==num){
        return previous;
    }
    else if(dp[num][signal]!=-1){
        return dp[num][signal];
    }
    else{
        if(arr[num].ff>=previous && arr[num].ss>=previous){
            if(signal==0){
                return dp[num][signal]=min(dp_solve(n,arr[num].ff,num+1,signal),dp_solve(n,arr[num].ss,num+1,signal^1));
            }
            else{
                return dp[num][signal]=min(dp_solve(n,arr[num].ff,num+1,signal^1),dp_solve(n,arr[num].ss,num+1,signal));
            }
        }
        else if(arr[num].ff>=previous){
            if(signal==0){
                return dp[num][signal]=dp_solve(n,arr[num].ff,num+1,signal);
            }
            else{
                return dp[num][signal]=dp_solve(n,arr[num].ff,num+1,signal^1);
            }
        }
        else if(arr[num].ss>=previous){
            if(signal==0){
                return dp[num][signal]=dp_solve(n,arr[num].ss,num+1,signal^1);
            }
            else{
                return dp[num][signal]=dp_solve(n,arr[num].ss,num+1,signal);
            }
        }
        else{
            return dp[num][signal]=INT_MAX;
        }
    }
}

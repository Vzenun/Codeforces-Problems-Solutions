...
// std::upper_bound(arr,arr+n,time)-arr;
...
bool binary_searc(ll * arr,ll i,ll j,ll key){
    if(i<j){
        ll m=(i+j)/2;
        if(arr[m]==key){
            return true;
            ...
            return binary_searc(arr,m+1,j,key);
        }
        else{
            return binary_searc(arr,i,m,key);
        }
    }
    else{
        if(arr[i]==key){
...
void solve_array(){
    ...
    //print_array(mrr,m);
    for(ll i=0;i<m;i++){
        if(binary_searc(arr,0,n-1,mrr[i])){
            cout<<(std::lower_bound(arr,arr+n,mrr[i])-arr)+1<<nn;
            //return;
        }
        else{
            cout<<(std::upper_bound(arr,arr+n,mrr[i])-arr)+1<<nn;
            //return
        }
    }
}

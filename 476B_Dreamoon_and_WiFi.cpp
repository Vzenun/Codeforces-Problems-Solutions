...
lld a1=0.0000000000;
void answer(ll value,ll counting,ll n,string s,lld probab){
    if(n==0){
        if(counting==value){
            a1+=probab;
        }
        ...
        if(s[n-1]=='+'){
            counting+=1;
            answer(value,counting,n-1,s,probab);
        }
        else if(s[n-1]=='-'){
            counting-=1;
            answer(value,counting,n-1,s,probab);
            ...
            answer(value,counting+1,n-1,s,probab*0.5);
            answer(value,counting-1,n-1,s,probab*0.5);
        }
    }
}
...
void solve_single(){
    ...
    answer(ans,0,s.size(),t,1);
    cout<<setprecision(10)<<a1<<nn;
}

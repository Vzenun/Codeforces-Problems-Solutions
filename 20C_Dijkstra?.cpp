...
void solve_graph(){
    ...
    while(!st.empty()){
        ...
        ll weight=k.first;
        ll vertex=k.second;
        rep(i,0,arr[vertex-1].size()){
            if(shortest_path[arr[vertex-1][i].first-1] > weight + arr[vertex-1][i].second){
                set<pll> :: iterator it;
                it=st.find(mp(shortest_path[arr[vertex-1][i].first-1],arr[vertex-1][i].first));
                if(it!=st.end()){
                    st.erase(it);
                }
                shortest_path[arr[vertex-1][i].first-1] = weight + arr[vertex-1][i].second;
                st.insert(mp(shortest_path[arr[vertex-1][i].first-1],arr[vertex-1][i].first));
                parent[arr[vertex-1][i].first-1]=vertex;
            }
        }
    }
    if(shortest_path[n-1]==1e18){
        cout<<-1<<nn;
    }
    else{
        ll v=n;

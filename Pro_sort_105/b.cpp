...
void seive(){seiv[0]=0;seiv[1]=1;for(ll i=2;i*i<2000003;i++){if(seiv[i]==0){seiv[i]=i;for(ll j=i*i;j<2000003;j=j+i){if(seiv[j]==0){seiv[j]=i;}}}}}
...
void solve_array(){
    ...
    for(ll i=0;i<n;i++){
        ...
        else if(seiv[arr[i]]<arr[i] && seiv[arr[i]]!=0){
            sum+=1;
        }
    }
    cout<<sum<<nn;

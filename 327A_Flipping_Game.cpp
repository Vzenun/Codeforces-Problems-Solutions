...
void solve_array(){
    ...
    for(ll i=0;i<n;i++){
        for(ll j=i;j<n;j++){
            if(i==j && arr[i]==0){
                max1=max(max1,num1+1);
            }
            else if(i==j && arr[i]==1){
                max1=max(max1,num1-1);
            }
            else if(i!=j && arr[i]==0 && arr[j]==0 && b[j]-b[i]==0){
                max1=max(max1,num1+a[j]-a[i]+1);
            }
            else if(i!=j && arr[i]==1 && arr[j]==0 && b[j]-b[i]==0){
                max1=max(max1,num1+a[j]-a[i]+1-b[j]+b[i]-1);
            }
            else if(i!=j && arr[i]==0 && arr[j]==1 && b[j]-b[i]==0){
                max1=max(max1,num1+a[j]-a[i]+1-b[j]+b[i]-1);
            }
            else{
                max1=max(max1,num1+a[j]-a[i]+1-b[j]+b[i]-1);
